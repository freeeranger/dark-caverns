...
public final class ArmorEffects {
    ...
    private static void applyShroomstoneBonus(Player player) {
        ...
        if (pieces > 0) {
            player.addEffect(new MobEffectInstance(MobEffects.JUMP, 20, pieces - 1, false, false, false));
        }
    }
    ...
    private static void applyScorchsteelBonus(Player player) {
        ...
        boolean stationary = data.contains(LAST_X)
        ...
        if (stillTicks >= DarkCavernsConfig.COMMON.scorchsteelStealthStandstillTicks.get()) {
            int effectDuration = switch (pieces) {
                case 2 -> 40;
                case 3 -> 60;
                case 4 -> 120;
                default -> 20;
            };
            player.addEffect(new MobEffectInstance(MobEffects.INVISIBILITY, effectDuration, 0, false, false, true));
        }
    }

    private static int countArmor(Player player, Item helmet, Item chestplate, Item leggings, Item boots) {
        int count = 0;
